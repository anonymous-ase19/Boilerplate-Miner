...
# diff_dir = "/Users/dayen/eclipse-workspace/api-mining-master/output/all/javax_xml_transform/pam/diff/"
# package_list = glob(diff_dir+"pattern_*.edgelist")
rood_dir = "/ssd1/dayen/APIList/data/survey_output/android_database_sqlite/"
# out_file = rood_dir + "wo_th_short_result_cluster_filtered.txt"
out_file = rood_dir + "short_result_cluster_filtered.txt"
# diff_dir = rood_dir + "wo_th_diff/"
# diff_dir = rood_dir + "diff/"
diff_dir = rood_dir + "back_diff/"
package_list = glob(diff_dir+ metric + "_*.edgelist")
sorted_list = sorted(package_list, key=lambda x: int(x.split(".edgelist")[0].split(metric+"_")[1]))

with open(out_file, "w") as out:
	for p in sorted_list:
		p_index = p.split("/")[-1].replace(".edgelist", "").replace(metric+"_", "")
		print(p_index)
		isTable = False
		file_dict = {}
		isContinue = False
		...
		if len(partition.keys()) == 0:
			...
			if len(partition.keys()) >= min_node:	
				...
				with open(diff_dir + "partition_" + metric + "_" + p_index + ".txt", "w") as f_out:
